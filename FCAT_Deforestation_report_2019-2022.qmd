...
# Read in drone validation
drone_fcat5 <- terra::rast(here("./reports/drone_fcat5_projected_cropped.tif"))


```

```{r pulling-in-vars, include=FALSE, ref.label=c("calculate-deforestation-rates", "calculate-land-cover-stats", "drone-accuracy")}
...
-   Annual deforestation rates in the areas surrounding the FCAT Reserve were **`r round(defor_compare %>% filter(model == "FCAT" & Area == "Overall") %>% pull(defor_perc_yr), 3) * 100`% / yr between 2019-2022,** approximately **`r round(defor_compare %>% filter(model == "FCAT"  & Area == "Overall") %>% pull(defor_perc_yr) / defor_compare %>% filter(model == "GFW Integrated Alerts" & Area == "Overall") %>% pull(defor_perc_yr), 1)`x higher** than best available contemporary estimates for this time period

-   We were able to predict with the presence of forest vs. non-forest with **`r round(drone_confusion_mat$overall[1], 3)*100`% accuracy**, tested against high-resolution drone imagery.
...
-   Tree crops like cacao and banana can wrongly be classified as forest (see @sec-cacao-as-forest), potentially leading to an overestimate of forest cover.
...
## Cacao classified as forest {#sec-cacao-as-forest}
...
::: column-page
```{r plot-drone-validation}
#| label: fig-drone-validation-image
#| fig-cap: "Cacao mis-classified as forest"
#| fig-subcap: 
#|   - "Drone imagery taken February 2023."
#|   - "Drone imagery taken February 2023 with overlay 2022 land cover classification."
#| layout-nrow: 1
#| layout-ncol: 2
#| layout-align: center
...
# Drone image
  plotRGB(drone_fcat5)
  sbar(50, xy = "bottomright", below = "meters", divs = 3, ticks = T, scaleby = 1)

# Drone + Overlay of land cover classification
   plot(cover2022, col = class_colors[-1],
       ext = ext(issues[7]),
       mar = c(0,0,0,0),
       type = "classes",
       levels = c("Forest", "Non-forest", "Cloud", "Shadow"),
       ...
       alpha = 1,
       add = FALSE)
  plotRGB(drone_fcat5, alpha = 150, add = TRUE)
...
::: column-page
```{r plot-cacao-as-restforestion}
#| label: fig-cacao-as-reforestation
#| fig-cap: "Cacao growth classified as reforestation"
#| fig-subcap: 
#|   - "2019 PlanetLab Satellite Imagery"
#|   - "2019 Land cover classification"
#|   - "2022 PlanetLab Satellite Imagery"
#|   - "2022 Land cover classification"
#|   - "2023 Drone imagery"
#|   - "2023 Drone imagery with reforestation / deforestation overlay"
#| layout-nrow: 3
#| layout-ncol: 2
#| layout-align: center
...
for(x in 7){
  ...
  # RGB in 2022 with deforestation overlay
  plot(crop(cover2022, issues[x]), 
       col = class_colors[-1],
       mar = c(0,0,1,0),
       axes = FALSE,
     type = "classes", 
  ...
  # Drone image
  plot(drone_fcat5, ext = ext(issues[7]))
  sbar(50, xy = "bottomright", below = "meters", divs = 3, ticks = T, scaleby = 1)
...
# Drone + Overlay of land cover classification
  ...
  plot(change_focal_refor,
       ...
       mar = c(0,0,0,0),
       type = "classes",
       levels = c("Deforestation", "Reforestation"),
       col = c("firebrick", "yellow"),     
       axes = FALSE,
       plg = list(x = "topright", bty ="o", bg = "white"),
       alpha = 0.6,
       add = TRUE)
...
More generally, detecting 'reforestation' seems to be more challenging that detecting deforestation, potentially leading to more false positives. [@fig-forest-change] shows where areas in green were predicted by the model to be cases of reforestation- transitioned from non-forest in 2019 to forest in 2022.
...
```{r drone-accuracy, echo=FALSE}

## Load in drone validation shape files
  drone_val <- terra::vect(here("./class_shapefiles/2023_02 drone validation classes.shp"))

  # Assign ids
  drone_val$id <- 1:length(drone_val)

# # Compare areas and counts of each class

  # plot(drone_val, col = drone_val$class)

  # training_count <- tibble(class = drone_val$class,
  #                                    area_ha = expanse(drone_val)/10000) %>%
  #                           group_by(class) %>%
  #                           summarize(area_ha = sum(area_ha),
  #                                     number_of_polygons = length(class)) %>%
  #                 mutate(class = factor(class, labels = c("forest", "non-forest")))
  # 
  # # Print out table
  # knitr::kable(training_count %>% filter(class != "built environment"))

# Extract predicted class values
pred <-  terra::extract(cover2022, drone_val, 
                        xy = FALSE, exact = FALSE, 
                        touches = TRUE) 

# summary(pred)

pred <- left_join(pred, values(drone_val), by = c("ID" = "id"))
pred <- pred %>%
  rename(pred_class = lyr1, true_class = class)

## Filter out shadows and clouds
pred <- pred %>%
  filter(!(pred_class %in% c(3,4)))


## Accuracy - take top probability for each pixel
# sum(pred$pred_class == pred$true_class) / nrow(pred)

# Calculate
drone_confusion_mat <- caret::confusionMatrix(factor(pred$true_class, 
                                        levels(factor(pred$pred_class))),
                                 factor(pred$pred_class, 
                                        levels(factor(pred$pred_class))),
                                 mode = "prec_recall")
```
...
Overall pixel-level model accuracy against this independent validation set was: `r round(drone_confusion_mat$overall[1], 3)*100`%. More detailed model performance statistics are below:

```{r echo=FALSE}


# Print confusion matrix
print(drone_confusion_mat)
```
