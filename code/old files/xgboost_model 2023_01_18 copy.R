...
## Test out running model without NIR column

## After making predictions across raster, replace cloud and shadow with the output from Planet UDM2
...
path_to_images = "../../../Satellite imagery/Planet imagery/August 2022/FCAT_Reserve_-_August_2022_psscene_analytic_sr_udm2/files/"
...
## Merge rasters
ras <- raster::merge(ras_1, ras_2)
udm <- raster::merge(udm_1, udm_2)
...
# Read in traing files for each class ------------------------------------------
...
for(x in 1:4)  {
  ...
  newdata <- as.matrix(ras_dat[cut_starts[x]:cut_ends[x], c("blue", "red", "green")])
