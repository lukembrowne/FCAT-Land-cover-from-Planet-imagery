...
# Load in merged raster ---------------------------------------------------
  ...
  # 8 band raster
  path_to_raster <- "../../../Satellite imagery/Planet imagery/2022_12/8_band_-_2022_12_psscene_analytic_8b_sr_udm2/2022_12 - 8 band merged.tif"
...
# Read in training files for each class ------------------------------------------
  ...
  # Set class names
  class_names <- c("agriculture", "cloud", "forest", "shadow")

  path_to_db <- "./class shapefiles/2022_12 classes.gpkg"
  st_layers(path_to_db)
  
  agriculture <- st_read(path_to_db, layer = "Agriculture")
  cloud <- st_read(path_to_db, layer = "Cloud")
  # development <- st_read(path_to_db, layer = "Development")
  forest <- st_read(path_to_db, layer = "Forest")
  shadow <-st_read(path_to_db, layer = "Shadow")
  
  plot(forest, col = "green")
  plot(cloud, add = TRUE, col = "blue")
  plot(agriculture, add = TRUE, col = "brown")
  # plot(development, add = TRUE, col = "purple")
  plot(shadow, add = TRUE, col = "grey90")

  
  # Compare areas of each class
  sum(st_area(agriculture))/10000
  sum(st_area(cloud))/10000
  # sum(st_area(development))/10000
  sum(st_area(forest))/10000
  sum(st_area(shadow))/10000
...
# Extract data for each class as a dataframe ------------------------------------
  ...
  dat_agriculture <- raster::extract(ras, agriculture, df = TRUE)
  dat_agriculture$class = "agriculture"
  dat_agriculture <- dat_agriculture %>% filter(!is.na(blue))
...
# set.seed(42)
## Run final model
boost <- xgboost(data =  as.matrix(dat_train[, band_names]),
                 ...
                 nrounds = 1000, # Do 500 or so for real model
...
### SAVE MODEL OUTPUT
# save(boost, band_names, class_names, path_to_raster,
#     file =  paste0("./output/model_run - 8 band December 2022 ", Sys.Date(), ".Rdata"))
...
pred = predict(boost,
               newdata = as.matrix(dat_test[, band_names]),
               reshape = TRUE)
...
# agriculture <- read_sf("./class shapefiles/agriculture class.shp")
